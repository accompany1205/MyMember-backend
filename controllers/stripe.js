...
exports.create = async (req, res) => {
    ...
    let isExist = await candidate_stripe.find({ candidate: stripeBody.candidate })
    try {
        if (isExist.length) {
            if (req.file) {
    ...
    try {
        if (isExist.length) {
            if (req.file) {
                ...
                await stripe.updatedOne({ _id: stripeId, $and: [{ userId: userId }, { adminId: adminId }] }, { $set: stripeBody })
...
exports.remove = async (req, res) => {
    try {
        ...
        const adminId = req.params.adminId
        const userId = req.params.userId;
        stripe.remove({ _id: stripeId, $and: [{ userId: userId }, { adminId: adminId }] },
            async (err, data) => {
                if (err) {
                    ...
                    await candidate_stripe.updateOne({ "stripes": stripeId }, { $pull: { "stripes": stripeId } },
                        function (err, data) {
                            if (err) {
