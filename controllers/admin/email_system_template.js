...
exports.add_template = async (req, res) => {
    const counts = await addTemp.find({ folderId: req.params.folderId }).countDocuments()
                    ...
                    try {
                        cron.schedule(`59 23 ${scheduleDateOfMonth} ${scheduleMonth} ${scheduleDay}`, async function () {
                            ...
                            sgMail.send_via_sendgrid(emailData).then((data) => {
                                addTemp.findByIdAndUpdate(templateId, { is_Sent: true }, async (er, data) => {
                                    if (er) {
...
exports.single_temp_update_status = (req, res) => {
    if (req.body.email_status == true) {
        addTemp.updateOne({ _id: req.params.tempId }, { $set: { email_status: true } }, (err, resp) => {
            if (err) {
        ...
        addTemp.updateOne({ _id: req.params.tempId }, { $set: { email_status: false } }, (err, resp) => {
            if (err) {
...
exports.swapAndUpdate_template = async (req, res) => {
    ...
    if (req.body.length < 1) {
        ...
        const first = await addTemp.findByIdAndUpdate(ObjectIdOfupdateTo, { templete_Id: updateFrom })
        const second = await addTemp.findByIdAndUpdate(ObjectIdOfupdateFrom, { templete_Id: updateTO })
