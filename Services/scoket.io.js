  ...
  async init() {
    ...
    this.sConnection.on('connection', function (socket) {
      ...
      socket.on('push-notification', async (userId) => {
        ...
        if(users.sixtydays_expire_notification_setting_renewal){
          let now = new Date();
          let todaysDate = moment(now).format('YYYY/MM/DD')
          const afterThirty = new Date(now.setDate(now.getDate() + 30));
          ...
          console.log("thirty_days_expire-->", sixty_days_expire)
        ...
        if(users.nintydays_expire_notification_setting_renewal){
          let now = new Date();
          let todaysDate = moment(now).format('YYYY/MM/DD')
          ...
          console.log("thirty_days_expire-->", ninty_days_expire)
